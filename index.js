...
// reponse texts should be refactored into varibles for cleanliness but I cbf rn
setInterval(function setLatestEvent() {
        ...
        .then(response => {
            if (response.data[0].id === latestEventID){
                ...
                bot.sendMessage(process.env.CHAT_ID2, `${response.data[0].actor.display_login} pushed to repo ${response.data[0].repo.name} with ${response.data[0].payload.size} commits! \n\nMessage: ${response.data[0].payload.commits[0].message} \n\nLink: https://github.com/${response.data[0].repo.name}/commits/${response.data[0].payload.commits[0].sha}`);
                ...
                bot.sendMessage(process.env.CHAT_ID2, `${response.data[0].actor.display_login} ${response.data[0].payload.action} a pull request on repo: ${response.data[0].repo.name}. \n\nLink: ${response.data[0].payload.pull_request.html_url}`);
                ...
                bot.sendMessage(process.env.CHAT_ID2, `${response.data[0].actor.display_login} reviewed a pull request! \n\nLink: \n\n${response.data[0].payload.review.html_url}`);
